...
startnewendless:
	@echo "revision=$(revision)"
	dh_testdir
	@nextlocal=$(shell expr `echo $(revision) | sed 's/.*endless//'` + 1); \
	echo "nextlocal=$$nextlocal"; \
	nextrev=`echo $(revision) | sed "s/\(.*endless\).*/\1$$nextlocal/"`; \
	now="$(shell date -R)"; \
	echo "Creating new changelog set for $(release)-$$nextrev..."; \
	echo -e "$(src_pkg_name) ($(release)-$$nextrev) UNRELEASED; urgency=low\n" > $(DEBIAN)/changelog.new; \
	echo "  CHANGELOG: Do not edit directly. Autogenerated at release." >> \
		$(DEBIAN)/changelog.new; \
	echo "  CHANGELOG: Use the printchanges target to see the curent changes." \
		>> $(DEBIAN)/changelog.new; \
