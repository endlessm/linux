...
static u32 glcoex_ver_date_8822b_1ant = 20170113;
static u32 glcoex_ver_8822b_1ant = 0x41;
static u32 glcoex_ver_btdesired_8822b_1ant = 0x41;
 ...
 * *************************************************************/
static u8 halbtc8822b1ant_bt_rssi_state(struct btc_coexist *btcoexist,
...
static void halbtc8822b1ant_monitor_wifi_ctr(struct btc_coexist *btcoexist)
{
	...
	if (coex_sta->under_ips) {
		...
		coex_sta->crc_ok_cck =
			btcoexist->btc_read_2byte(btcoexist, 0xf04);
		coex_sta->crc_ok_11g =
			btcoexist->btc_read_2byte(btcoexist, 0xf14);
		coex_sta->crc_ok_11n =
			btcoexist->btc_read_2byte(btcoexist, 0xf10);
		coex_sta->crc_ok_11n_agg =
			btcoexist->btc_read_2byte(btcoexist, 0xf0c);

		coex_sta->crc_err_cck =
			btcoexist->btc_read_2byte(btcoexist, 0xf00) +
			btcoexist->btc_read_2byte(btcoexist, 0xf06);

		coex_sta->crc_err_11g =
			btcoexist->btc_read_2byte(btcoexist, 0xf16);
		coex_sta->crc_err_11n =
			btcoexist->btc_read_2byte(btcoexist, 0xf12);
		coex_sta->crc_err_11n_agg =
			btcoexist->btc_read_2byte(btcoexist, 0xf0e);
	}
...
static void
halbtc8822b1ant_update_wifi_channel_info(struct btc_coexist *btcoexist, u8 type)
{
					   ...
					   u32 reg_value)
{
	...
	if (bit_mask == 0xffffffff) {
		/* put write data */
		btcoexist->btc_write_4byte(btcoexist, 0x1704, reg_value);
		...
		btcoexist->btc_write_4byte(btcoexist, 0x1704, val);
...
static void halbtc8822b1ant_power_save_state(struct btc_coexist *btcoexist,
				    ...
				    bool force_exec, bool turn_on, u8 type)
{
	...
	if (turn_on) {
		...
		case 48:
			...
			break;
		case 49:
			halbtc8822b1ant_set_fw_pstdma(btcoexist, 0x55, 0x1e,
						      0x3, 0x10, 0x54);
			break;
		case 50:
						  ...
						  u8 wifi_status)
{
	...
	bt_rssi_state = halbtc8822b1ant_bt_rssi_state(btcoexist, 2, 28, 0);

	RT_TRACE(rtlpriv, COMP_BT_COEXIST, DBG_LOUD,
		 "[BTCoex], ********** (wifi connect acl busy) **********\n");

	if (bt_link_info->hid_only) {
		/* HID  + wifi connected idle or busy / 0x778=1@wifi slot */
		...
		/* A2DP  + wifi connected idle or busy */

		if (wifi_status == BT_8822B_1ANT_WIFI_STATUS_CONNECTED_IDLE) {
			btcoexist->btc_write_1byte_bitmask(btcoexist, 0x45e,
							   0x8, 0x1);
		...
		btcoexist->btc_write_1byte_bitmask(btcoexist, 0x45e, 0x8, 0x1);
		if (!wifi_busy) {
			/* a2dp glitch while wl idle,
			 * change the 0x6c0=5a5a5a5a->5555or aaaa55aa
			 */
			halbtc8822b1ant_coex_table_with_type(btcoexist,
							     NORMAL_EXEC, 0);
			halbtc8822b1ant_ps_tdma(btcoexist, NORMAL_EXEC, true,
			...
			/*if in open space, and ask wl tp,
			 *0x60=6135031111,55555555,aaaa55aa
			 */
			halbtc8822b1ant_coex_table_with_type(btcoexist,
							     NORMAL_EXEC, 1);
			halbtc8822b1ant_ps_tdma(btcoexist, NORMAL_EXEC, true,
		   ...
		   (bt_link_info->hid_exist &&
		    bt_link_info
			    ->pan_exist)) { /* PAN(OPP,FTP), HID+PAN(OPP,FTP)*/
...
halbtc8822b1ant_action_wifi_not_connected_scan(struct btc_coexist *btcoexist)
{
	...
	if (coex_dm->bt_status == BT_8822B_1ANT_BT_STATUS_ACL_BUSY) {
		...
		halbtc8822b1ant_action_bt_sco_hid_only_busy(
			btcoexist, BT_8822B_1ANT_WIFI_STATUS_CONNECTED_SCAN);
	} else {
		halbtc8822b1ant_ps_tdma(btcoexist, NORMAL_EXEC, false, 8);
...
void ex_halbtc8822b1ant_power_on_setting(struct btc_coexist *btcoexist)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	struct btc_board_info *board_info = &btcoexist->board_info;
	u8 u8tmp = 0x0;
...
void ex_halbtc8822b1ant_pre_load_firmware(struct btc_coexist *btcoexist) {}

void ex_halbtc8822b1ant_init_hw_config(struct btc_coexist *btcoexist,
				       bool wifi_only)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_init_coex_dm(struct btc_coexist *btcoexist)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_display_coex_info(struct btc_coexist *btcoexist)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	struct btc_board_info *board_info = &btcoexist->board_info;
	struct btc_bt_link_info *bt_link_info = &btcoexist->bt_link_info;
	...
	if (psd_scan->ant_det_try_count == 0) {
		...
		CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
			   ...
			   board_info->btdm_ant_num_by_ant_det,
			   board_info->btdm_ant_pos, rfe_type->rfe_module_type,
	...
	CL_SPRINTF(
		cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d / %d / %d / %d / %d",
		"SCO/HID/PAN/A2DP/Hi-Pri", bt_link_info->sco_exist,
		bt_link_info->hid_exist, bt_link_info->pan_exist,
		bt_link_info->a2dp_exist, bt_link_info->bt_hi_pri_link_exist);
	CL_PRINTF(cli_buf);
	...
	/*coex table type*/
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d",
		   "WL/BT Coex Table Type", coex_sta->coex_table_type);
	CL_PRINTF(cli_buf);
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%x/ 0x%x",
		   "0x778/0x6cc/IgnWlanAct", u8tmp[0], u32tmp[0],
		   coex_dm->cur_ignore_wlan_act);
	CL_PRINTF(cli_buf);

	/*LTE : WL/LTE coex table : 0xa0 */

	u32tmp[0] = halbtc8822b1ant_ltecoex_indirect_read_reg(btcoexist, 0xa0);

	/*LTE : BT/LTE coex table : 0xa4 */
	u32tmp[1] = halbtc8822b1ant_ltecoex_indirect_read_reg(btcoexist, 0xa4);
	...
	u32tmp[0] = halbtc8822b1ant_ltecoex_indirect_read_reg(btcoexist, 0xa8);
	/*LTE : WL/LTE break table : 0xac */
	u32tmp[1] = halbtc8822b1ant_ltecoex_indirect_read_reg(btcoexist, 0xac);
	/*LTE : LTE/WL break table : 0xb0 */
	u32tmp[2] = halbtc8822b1ant_ltecoex_indirect_read_reg(btcoexist, 0xb0);
	/*LTE : LTE/BT break table : 0xb4 */
	u32tmp[3] = halbtc8822b1ant_ltecoex_indirect_read_reg(btcoexist, 0xb4);

	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
		   "\r\n %-35s = 0x%x/ 0x%x/ 0x%x/ 0x%x",
		   "LTE Break Table W_L/B_L/L_W/L_B", u32tmp[0] & 0xffff,
		   u32tmp[1] & 0xffff, u32tmp[2] & 0xffff, u32tmp[3] & 0xffff);
	CL_PRINTF(cli_buf);
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%x/ 0x%x",
		   "0xcb4/0xcbd/0x1991", u8tmp[1], u8tmp[2], u8tmp[3]);
	CL_PRINTF(cli_buf);
...
void ex_halbtc8822b1ant_ips_notify(struct btc_coexist *btcoexist, u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_lps_notify(struct btc_coexist *btcoexist, u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_scan_notify(struct btc_coexist *btcoexist, u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	bool wifi_connected = false;
	bool wifi_under_5g = false;
...
void ex_halbtc8822b1ant_scan_notify_without_bt(struct btc_coexist *btcoexist,
					       u8 type)
{
	bool wifi_under_5g = false;
...
void ex_halbtc8822b1ant_switchband_notify(struct btc_coexist *btcoexist,
					  u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_switchband_notify_without_bt(
...
void ex_halbtc8822b1ant_connect_notify(struct btc_coexist *btcoexist, u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	bool wifi_connected = false;
...
void ex_halbtc8822b1ant_media_status_notify(struct btc_coexist *btcoexist,
					    u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	bool wifi_under_b_mode = false;
...
void ex_halbtc8822b1ant_specific_packet_notify(struct btc_coexist *btcoexist,
					       u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_bt_info_notify(struct btc_coexist *btcoexist,
				       u8 *tmp_buf, u8 length)
{
	...
	if (rsp_source != BT_INFO_SRC_8822B_1ANT_WIFI_FW) {
		...
		/* check BIT2 ==> check if bt is under inquiry or page scan */
	...
	} else if (bt_info == BT_INFO_8822B_1ANT_B_CONNECTION) {
...
void ex_halbtc8822b1ant_rf_status_notify(struct btc_coexist *btcoexist, u8 type)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_halt_notify(struct btc_coexist *btcoexist)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_pnp_notify(struct btc_coexist *btcoexist, u8 pnp_state)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	bool wifi_under_5g = false;
...
void ex_halbtc8822b1ant_coex_dm_reset(struct btc_coexist *btcoexist)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
...
void ex_halbtc8822b1ant_periodical(struct btc_coexist *btcoexist)
{
	struct rtl_priv *rtlpriv = btcoexist->adapter;
	bool wifi_busy = false;
	...
	if (wifi_busy) {
		...
		halbtc8822b1ant_post_state_to_bt(
			btcoexist, BT_8822B_1ANT_SCOREBOARD_WLBUSY, true);
		...
		halbtc8822b1ant_post_state_to_bt(
			btcoexist, BT_8822B_1ANT_SCOREBOARD_WLBUSY, false);
	}
...
void ex_halbtc8822b1ant_antenna_detection(struct btc_coexist *btcoexist,
					  u32 cent_freq, u32 offset, u32 span,
					  u32 seconds)
{
}

void ex_halbtc8822b1ant_antenna_isolation(struct btc_coexist *btcoexist,
					  u32 cent_freq, u32 offset, u32 span,
					  u32 seconds)
{
}

void ex_halbtc8822b1ant_psd_scan(struct btc_coexist *btcoexist, u32 cent_freq,
				 u32 offset, u32 span, u32 seconds)
{
}

void ex_halbtc8822b1ant_display_ant_detection(struct btc_coexist *btcoexist) {}

void ex_halbtc8822b1ant_dbg_control(struct btc_coexist *btcoexist, u8 op_code,
				    u8 op_len, u8 *pdata)
{
}
