...
u16 rtl8822be_get_available_desc(struct ieee80211_hw *hw, u8 q_idx)
{
	...
	if (ring->avl_desc)	/* if available > 0, don't update RW register */
		return ring->avl_desc;
	...
	if (current_tx_read_point == current_tx_write_point)
		point_diff = TX_DESC_NUM_8822B - 1;
	else if (current_tx_read_point < current_tx_write_point)
		point_diff = TX_DESC_NUM_8822B -
			     (current_tx_write_point - current_tx_read_point) -
			     1;
	else
		point_diff = current_tx_read_point - current_tx_write_point - 1;

	if (point_diff == 0)
		RT_TRACE(rtlpriv, COMP_SEND, DBG_DMESG, "CR:%d,CW:%d\n",
...
#if (RTL8822BE_SEG_NUM == 0)
	u8 segmentnum = 2;
#elif (RTL8822BE_SEG_NUM == 1)
	u8 segmentnum = 4;
#elif (RTL8822BE_SEG_NUM == 2)
	u8 segmentnum = 8;
#endif
...
#if (DMA_IS_64BIT == 1)
		SET_TXBUFFER_DESC_ADD_HIGH_WITH_OFFSET(tx_bd_desc, i, 0);
#endif
	}
			       ...
			       struct rtl_tcb_desc *ptcb_desc)
{
	...
	RT_TRACE(rtlpriv, COMP_SEND, DBG_TRACE,
		 "rtl8822be_bw_mapping, current_chan_bw %d, packet_bw %d\n",
	...
	if (istx) {
		...
		case HW_DESC_OWN: {
			...
			if (ring->avl_desc >= 1) {
				...
				RT_TRACE(
					rtlpriv, COMP_SEND, DBG_DMESG,
					"critical error! ring->avl_desc == 0\n");
			}
...
u32 rtl8822be_get_desc(u8 *pdesc, bool istx, u8 desc_name)
{
	...
	if (istx) {
		...
		case HW_DESC_TXBUFF_ADDR_HI:
			ret = GET_TXBUFFER_DESC_ADDR_HIGH(pbd_desc, 1);
			break;
#endif
		default:
				 ...
				 u16 index)
{
	...
	if (index == ring->cur_tx_rp) {
		...
		WARN_ONCE(ring->cur_tx_wp != cur_tx_wp,
			  "ring->cur_tx_wp(%X) != cur_tx_wp(%X)\n",
			  ring->cur_tx_wp, cur_tx_wp);

		ring->cur_tx_wp = cur_tx_wp;	/* they should be the same */
		ring->cur_tx_rp = cur_tx_rp;
		ring->avl_desc = ((cur_tx_rp > cur_tx_wp) ?
				  (cur_tx_rp - cur_tx_wp) :
				  (TX_DESC_NUM_8822B - cur_tx_wp + cur_tx_rp));
