...
_rtl8822be_phy_convert_txpower_limit_to_power_index(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(
		rtlpriv, COMP_INIT, DBG_TRACE,
		"=====> _rtl8822be_phy_convert_txpower_limit_to_power_index()\n");
...
static void _rtl8822be_phy_init_txpower_limit(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(rtlpriv, COMP_INIT, DBG_LOUD,
		 "=====> _rtl8822be_phy_init_txpower_limit()!\n");
...
_rtl8822be_phy_convert_txpower_dbm_to_relative_value(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(
		rtlpriv, COMP_POWER, DBG_TRACE,
		"<===_rtl8822be_phy_convert_txpower_dbm_to_relative_value()\n");
}
...
void rtl8822be_phy_sw_chnl_callback(struct ieee80211_hw *hw)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_hal *rtlhal = rtl_hal(rtl_priv(hw));
	struct rtl_phy *rtlphy = &rtlpriv->phy;
	...
	for (path = RF90_PATH_A; path < rtlphy->num_total_rfpath; path++) {
		if (channel >= 36 && channel <= 64)
			...
			data = 0x000;
		rtl8822be_phy_set_rf_reg(
			hw, path, RF_CHNLBW,
			BIT(18) | BIT(17) | BIT(16) | BIT(9) | BIT(8), data);

		rtl8822be_phy_set_rf_reg(hw, path, RF_CHNLBW, BMASKBYTE0,
					 channel);
		...
		if (channel > 14) {
			if (rtlhal->hw_type == HARDWARE_TYPE_RTL8822BE) {
				if (channel >= 36 && channel <= 64)
					...
					data = 0x110E9;
				rtl8822be_phy_set_rf_reg(hw, path, RF_APK,
							 BRFREGOFFSETMASK,
							 data);
			}
		}
