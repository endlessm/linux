...
static bool _rtl8822be_init_trxbd(struct ieee80211_hw *hw)
{
	...
	rtl_write_word(rtlpriv, REG_RX_RXBD_NUM_8822B,
		       RX_DESC_NUM_8822BE |
			       ((RTL8822BE_SEG_NUM << 13) & 0x6000) | 0x0000);
#endif
...
static void _rtl8822be_enable_aspm_back_door(struct ieee80211_hw *hw)
{
	...
	rtl_write_word(rtlpriv, REG_DBI_FLAG_V1_8822B, 0x78);
	rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2, 0x2);
	tmp8 = rtl_read_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2);
	count = 0;
	while (tmp8 && count < 20) {
		udelay(10);
		tmp8 = rtl_read_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2);
	...
	if (tmp8 == 0) {
		tmp32 = rtl_read_dword(rtlpriv, REG_DBI_RDATA_V1_8822B);
		if ((tmp32 & 0xff00) != 0x2000) {
			tmp32 &= 0xffff00ff;
			rtl_write_dword(rtlpriv, REG_DBI_WDATA_V1_8822B,
					tmp32 | BIT(13));
			rtl_write_word(rtlpriv, REG_DBI_FLAG_V1_8822B, 0xf078);
			rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2, 0x1);

			tmp8 = rtl_read_byte(rtlpriv,
					     REG_DBI_FLAG_V1_8822B + 2);
			count = 0;
			while (tmp8 && count < 20) {
				udelay(10);
				tmp8 = rtl_read_byte(rtlpriv,
	...
	rtl_write_word(rtlpriv, REG_DBI_FLAG_V1_8822B, 0x70c);
	rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2, 0x2);
	tmp8 = rtl_read_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2);
	count = 0;
	while (tmp8 && count < 20) {
	...
	if (tmp8 == 0) {
		...
		rtl_write_dword(rtlpriv, REG_DBI_WDATA_V1_8822B,
				tmp32 | BIT(31));
		rtl_write_word(rtlpriv, REG_DBI_FLAG_V1_8822B, 0xf70c);
		rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2, 0x1);
	}
	...
	rtl_write_word(rtlpriv, REG_DBI_FLAG_V1_8822B, 0x718);
	rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2, 0x2);
	tmp8 = rtl_read_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2);
	count = 0;
	while (tmp8 && count < 20) {
	...
	if (ppsc->support_backdoor || (tmp8 == 0)) {
		...
		rtl_write_dword(rtlpriv, REG_DBI_WDATA_V1_8822B,
				tmp32 | BIT(11) | BIT(12));
		rtl_write_word(rtlpriv, REG_DBI_FLAG_V1_8822B, 0xf718);
		rtl_write_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2, 0x1);
	}
	tmp8 = rtl_read_byte(rtlpriv, REG_DBI_FLAG_V1_8822B + 2);
	count = 0;
...
void rtl8822be_set_qos(struct ieee80211_hw *hw, int aci)
{
	...
	ac_param = rtl_get_hal_edca_param(&mac->edca_param[aci]);
				   ...
				   struct ieee80211_sta *sta, u8 rssi_level)
{
	...
	if (rtlpriv->dm.useramask)
		rtl8822be_update_hal_rate_mask(hw, sta, rssi_level);
}
