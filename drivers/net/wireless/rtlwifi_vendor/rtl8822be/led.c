...
void rtl8822be_sw_led_on(struct ieee80211_hw *hw, struct rtl_led *pled)
{
	...
	case LED_PIN_GPIO0:
		break;
	case LED_PIN_LED0:
		ledcfg = rtl_read_dword(rtlpriv, REG_GPIO_PIN_CTRL_8822B);
		ledcfg &= ~BIT(13); /* out value */
		ledcfg |= BIT(21); /* input/output */
		ledcfg &= ~BIT(29); /* data/int */

		rtl_write_dword(rtlpriv, REG_GPIO_PIN_CTRL_8822B, ledcfg);
