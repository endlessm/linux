...
u8 odm_get_auto_channel_select_result(void *dm_void, u8 band)
{
	...
	if (band == ODM_BAND_2_4G) {
		ODM_RT_TRACE(
			dm, ODM_COMP_ACS,
			"[struct acs_info] odm_get_auto_channel_select_result(): clean_channel_2g(%d)\n",
			acs->clean_channel_2g);
		result = (u8)acs->clean_channel_2g;
	} else {
		ODM_RT_TRACE(
			dm, ODM_COMP_ACS,
			"[struct acs_info] odm_get_auto_channel_select_result(): clean_channel_5g(%d)\n",
...
void odm_auto_channel_select(void *dm_void, u8 channel)
{
	...
	if (channel >= 1 && channel <= 14) {
		...
		ODM_RT_TRACE(
			dm, ODM_COMP_ACS,
			"(1)odm_auto_channel_select(): 2G: clean_channel_2g = %d, max_score = %d\n",
