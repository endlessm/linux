...
halmac_mount_api_88xx(struct halmac_adapter *halmac_adapter)
{
	...
	pr_info("HALMAC_MAJOR_VER_88XX = %x\n", HALMAC_MAJOR_VER_88XX);
	pr_info("HALMAC_PROTOTYPE_88XX = %x\n", HALMAC_PROTOTYPE_VER_88XX);
	pr_info("HALMAC_MINOR_VER_88XX = %x\n", HALMAC_MINOR_VER_88XX);
	pr_info("HALMAC_PATCH_VER_88XX = %x\n", HALMAC_PATCH_VER_88XX);
			      ...
			      u8 *hamacl_fw, u32 halmac_fw_size)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
			"halmac_download_firmware_88xx start!!\n");
				   ...
				   u32 halmac_fw_size)
{
	...
	HALMAC_RT_TRACE(
		driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
		"[TRACE]halmac_free_download_firmware_88xx ==========>\n");
			 ...
			 union halmac_wlan_addr *hal_address)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
			"[TRACE]halmac_cfg_mac_addr_88xx ==========>\n");
...
halmac_init_wmac_cfg_88xx(struct halmac_adapter *halmac_adapter)
{
	...
	HALMAC_REG_WRITE_32(halmac_adapter,
			    REG_WL2LTECOEX_INDIRECT_ACCESS_CTRL_V1, 0xC00F0038);
			 ...
			 enum halmac_trx_mode mode)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
			"halmac_init_mac_cfg_88xx ==========>mode = %d\n",
			mode);
			       ...
			       enum halmac_wireless_mode wireless_mode)
{
	...
	HALMAC_RT_TRACE(
		driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
		"halmac_cfg_operation_mode_88xx ==========>wireless_mode = %d\n",
		wireless_mode);
		      ...
		      enum halmac_pri_ch_idx pri_ch_idx, enum halmac_bw bw)
{
	...
	HALMAC_RT_TRACE(
		driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
		"halmac_cfg_ch_bw_88xx ==========>ch = %d, idx=%d, bw=%d\n",
			   ...
			   enum halmac_pri_ch_idx pri_ch_idx)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
			"halmac_cfg_pri_ch_idx_88xx ==========> idx=%d\n",
			pri_ch_idx);
					  ...
					  enum halmac_bw bw)
{
	...
	case HALMAC_BW_5:
		break;
	default:
		pr_err("halmac_cfg_bw_88xx switch case not support\n");
		break;
	}
			   ...
			   enum halmac_efuse_read_cfg cfg)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_INIT, DBG_DMESG,
			"halmac_dump_efuse_map_88xx ==========>cfg=%d\n", cfg);
				   ...
				   enum halmac_efuse_read_cfg cfg)
{
	...
	HALMAC_RT_TRACE(
		driver_adapter, HALMAC_MSG_EFUSE, DBG_DMESG,
		"halmac_dump_logical_efuse_map_88xx ==========>cfg = %d\n",
		cfg);
	...
	*process_status = HALMAC_CMD_PROCESS_IDLE;
	...
	if (halmac_adapter->hal_efuse_map_valid) {
		*process_status = HALMAC_CMD_PROCESS_DONE;
		...
		if (!eeprom_map) {
			pr_err("halmac allocate local eeprom map Fail!!\n");
			return HALMAC_RET_MALLOC_FAIL;
		}
			  ...
			  u8 full_fifo)
{
	...
	if (halmac_adapter->halmac_state.dlfw_state == HALMAC_DLFW_NONE) {
		pr_err("halmac_cfg_parameter_88xx Fail due to DLFW NONE!!\n");
		return HALMAC_RET_DLFW_FAIL;
	}
			    ...
			    struct halmac_phy_parameter_info *para_info)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
			"[TRACE]halmac_update_datapack_88xx ==========>\n");
			 ...
			 enum halmac_drv_info halmac_drv_info)
{
	...
	default:
		...
		pr_err("halmac_cfg_drv_info_88xx error = %x\n", status);
		return status;
	}
				  ...
				  u8 *cur_desc)
{
	...
	if (!cur_desc) {
		pr_err("halmac_fill_txdesc_check_sum_88xx NULL PTR");
		return HALMAC_RET_NULL_POINTER;
	}
		     ...
		     enum halmac_bw bw, u8 txbf_en)
{
	...
	if (txbf_en) {
		...
		case HALMAC_BW_20:
			...
			break;
		default:
			pr_err("halmac_cfg_txbf_88xx invalid TXBF BW setting 0x%x of userid %d\n",
			       bw, userid);
	...
	case 1:
		...
		break;
	default:
		pr_err("halmac_cfg_txbf_88xx invalid userid %d\n", userid);
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}

	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_SND, DBG_DMESG,
			"halmac_cfg_txbf_88xx, txbf_en = %x <==========\n",
			txbf_en);
	...
	if (cfgmu->role == HAL_BFEE) {
		...
		if (!cfgmu->mu_tx_en) {
			...
			HALMAC_RT_TRACE(
				...
				"halmac_cfg_mumimo_88xx disable mu tx <==========\n");
			return HALMAC_RET_SUCCESS;
		}
			 ...
			 enum halmac_data_rate datarate)
{
	...
	case HAL_BFEE:
		...
		break;
	default:
		pr_err("halmac_cfg_sounding_88xx invalid role\n");
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}
			 ...
			 enum halmac_snd_role role)
{
	...
	case HAL_BFEE:
		...
		break;
	default:
		pr_err("halmac_del_sounding_88xx invalid role\n");
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}
			       ...
			       u16 paid)
{
	...
	case 1:
		...
		break;
	default:
		pr_err("halmac_su_bfee_entry_init_88xx invalid userid %d\n",
		       userid);
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}
			       ...
			       struct halmac_su_bfer_init_para *su_bfer_init)
{
	...
	case 1:
		...
		break;
	default:
		pr_err("halmac_su_bfer_entry_init_88xx invalid userid %d\n",
		       su_bfer_init->userid);
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}
...
halmac_su_bfee_entry_del_88xx(struct halmac_adapter *halmac_adapter, u8 userid)
{
	...
	case 1:
		...
		break;
	default:
		pr_err("halmac_su_bfee_entry_del_88xx invalid userid %d\n",
		       userid);
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}
...
halmac_su_bfer_entry_del_88xx(struct halmac_adapter *halmac_adapter, u8 userid)
{
	...
	case 1:
		...
		break;
	default:
		pr_err("halmac_su_bfer_entry_del_88xx invalid userid %d\n",
		       userid);
		return HALMAC_RET_INVALID_SOUNDING_SETTING;
	}
			...
			struct halmac_ch_info *ch_info)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
			"[TRACE]halmac_add_ch_info_88xx ==========>\n");

	if (halmac_adapter->halmac_state.dlfw_state != HALMAC_GEN_INFO_SENT) {
		pr_err("[ERR]halmac_add_ch_info_88xx: gen_info is not send to FW!!!!\n");
	...
	if (ch_sw_info->extra_info_en == 1) {
		pr_err("[ERR]halmac_add_ch_info_88xx: construct sequence wrong!!\n");
		return HALMAC_RET_CH_SW_SEQ_WRONG;
	}

	if (ch_sw_info->avai_buf_size < 4) {
		pr_err("[ERR]halmac_add_ch_info_88xx: no available buffer!!\n");
			      ...
			      struct halmac_ch_extra_info *ch_extra_info)
{
	...
	if (!ch_sw_info->ch_info_buf) {
		pr_err("halmac_add_extra_ch_info_88xx: NULL==ch_sw_info->ch_info_buf!!\n");
		return HALMAC_RET_CH_SW_SEQ_WRONG;
	}

	if (ch_sw_info->extra_info_en == 0) {
		pr_err("halmac_add_extra_ch_info_88xx: construct sequence wrong!!\n");
		return HALMAC_RET_CH_SW_SEQ_WRONG;
	}
	...
	if (ch_sw_info->avai_buf_size <
		 ...
		 * are totally 2Byte
		 */
		pr_err("halmac_add_extra_ch_info_88xx: no available buffer!!\n");
		return HALMAC_RET_CH_SW_NO_BUF;
	}
			   ...
			   struct halmac_ch_switch_option *cs_option)
{
	...
	HALMAC_RT_TRACE(
		...
		"halmac_ctrl_ch_switch_88xx  cs_option->switch_en = %d==========>\n",
		cs_option->switch_en);
	...
	if (cs_option->switch_en) {
		if (state_scan != HALMAC_SCAN_CMD_CONSTRUCT_CONSTRUCTING) {
			HALMAC_RT_TRACE(
				driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
				"halmac_ctrl_ch_switch_88xx(on)  invalid in state %x\n",
				state_scan);
			return HALMAC_RET_ERROR_STATE;
		}
	} else {
		if (state_scan != HALMAC_SCAN_CMD_CONSTRUCT_BUFFER_CLEARED) {
			HALMAC_RT_TRACE(
				driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
				"halmac_ctrl_ch_switch_88xx(off)  invalid in state %x\n",
				state_scan);
			return HALMAC_RET_ERROR_STATE;
		}
	}
			      ...
			      struct halmac_general_info *general_info)
{
	...
	if (halmac_adapter->halmac_state.dlfw_state == HALMAC_DLFW_NONE) {
		pr_err("halmac_send_general_info_88xx Fail due to DLFW NONE!!\n");
		return HALMAC_RET_DLFW_FAIL;
	}
			 ...
			 u8 *data, u32 *size)
{
	...
	case HALMAC_FEATURE_PSD:
		...
		break;
	default:
		pr_err("halmac_query_status_88xx invalid feature id %d\n",
		       feature_id);
			  ...
			  enum halmac_feature_id feature_id)
{
	...
	case HALMAC_FEATURE_ALL:
		...
		break;
	default:
		pr_err("halmac_reset_feature_88xx invalid feature id %d\n",
		       feature_id);
		return HALMAC_RET_INVALID_FEATURE_ID;
	}
			...
			enum halmac_la_mode la_mode)
{
	...
	HALMAC_RT_TRACE(driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
			"halmac_cfg_la_mode_88xx ==========>la_mode = %d\n",
			la_mode);
	...
	enum halmac_rx_fifo_expanding_mode rx_fifo_expanding_mode)
{
	...
	HALMAC_RT_TRACE(
		driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
		"halmac_cfg_rx_fifo_expanding_mode_88xx ==========>rx_fifo_expanding_mode = %d\n",
			 ...
			 enum halmac_hw_id hw_id, void *pvalue)
{
	...
	if (!pvalue) {
		pr_err("halmac_get_hw_value_88xx (!pvalue)==========>\n");
		return HALMAC_RET_NULL_POINTER;
	}
			 ...
			 enum halmac_hw_id hw_id, void *pvalue)
{
	...
	if (!pvalue) {
		pr_err("halmac_set_hw_value_88xx (!pvalue)==========>\n");
		return HALMAC_RET_NULL_POINTER;
	}
				...
				enum halmac_drv_rsvd_pg_num pg_num)
{
	...
	HALMAC_RT_TRACE(
		driver_adapter, HALMAC_MSG_H2C, DBG_DMESG,
		"halmac_cfg_drv_rsvd_pg_num_88xx ==========>pg_num = %d\n",
		pg_num);
