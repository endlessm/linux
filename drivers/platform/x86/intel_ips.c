...
int i915_bdw_enabled = 0;
EXPORT_SYMBOL(i915_bdw_enabled);
 ...
 */
static bool ips_get_i915_bdw_syms(struct ips_driver *ips)
{
	ips->read_mch_val = symbol_get(i915_bdw_read_mch_val);
	if (!ips->read_mch_val)
		goto out_err;
	ips->gpu_raise = symbol_get(i915_bdw_gpu_raise);
	if (!ips->gpu_raise)
		goto out_put_mch;
	ips->gpu_lower = symbol_get(i915_bdw_gpu_lower);
	if (!ips->gpu_lower)
		goto out_put_raise;
	ips->gpu_busy = symbol_get(i915_bdw_gpu_busy);
	if (!ips->gpu_busy)
		goto out_put_lower;
	ips->gpu_turbo_disable = symbol_get(i915_bdw_gpu_turbo_disable);
	if (!ips->gpu_turbo_disable)
		goto out_put_busy;
...
out_put_busy:
	symbol_put(i915_bdw_gpu_busy);
out_put_lower:
	symbol_put(i915_bdw_gpu_lower);
out_put_raise:
	symbol_put(i915_bdw_gpu_raise);
out_put_mch:
	symbol_put(i915_bdw_read_mch_val);
out_err:
	return false;
}
...
{
	if (ips_get_i915_bdw_syms(ips) != false)
		return true;
