				...
				int scope)
{
	 ...
	 *
	 * Since we cannot rely on the order of the cpucaps we cannot rely on
	 * the cpus_have_*cap() helpers to detect the erratum on the system.
	 * However, since affected CPUs are always in a homoegeneous
	 * configuration we could rely on this_cpu_has_cap()
	 */
